...
{
    Task<IEnumerable<BookResponseDto>> BrowseAsync(Guid? currentUserId, string? search, string? genre, string? language, CancellationToken cancellationToken = default);
    Task<BookResponseDto> GetByIdAsync(Guid? currentUserId, Guid bookId, CancellationToken cancellationToken = default);
